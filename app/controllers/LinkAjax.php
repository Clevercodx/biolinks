...
class LinkAjax extends Controller {
    ...
    private function update_biolink() {
        ...
        $image_allowed_extensions = [
            'seo_image' => $this->links_types['biolink']['whitelisted_seo_image_extensions'],
            'favicon' => $this->links_types['biolink']['whitelisted_favicon_image_extensions'],
            'background' => $this->links_types['biolink']['whitelisted_background_image_extensions'],
        ];
        $image = [
    ...
    private function update_vcard() {
        ...
        $biolink_block_ajax = (new \Altum\Controllers\BiolinkBlockAjax());
        $biolink_block_ajax->add_params(['user' => $this->user]);
        $db_vcard_avatar = $biolink_block_ajax->handle_file_upload($link->settings->vcard_avatar, 'vcard_avatar', 'vcard_avatar_remove', \Altum\Uploads::get_whitelisted_file_extensions('vcards_avatars'), \Altum\Uploads::get_path('vcards_avatars'), 0.75);
        ...
        $settings['vcard_phone'] = $_POST['vcard_phone'] = mb_substr(input_clean($_POST['vcard_phone']), 0, $this->links_types['vcard']['fields']['phone']['max_length']);
