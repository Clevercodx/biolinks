        ...
        if(!empty($_POST)) {
                    ...
                    if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                        try {
                            ...
                            $s3->deleteObject([
                                ...
                                'Key' => UPLOADS_URL_PATH . 'biolinks_themes/' . $biolink_theme->image,
                            ]);

                            /* Upload image */
                            $result = $s3->putObject([
                                'Bucket' => settings()->offload->storage_name,
                                'Key' => UPLOADS_URL_PATH . 'biolinks_themes/' . $image_new_name,
                ...
                if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                    ...
                    if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                        try {
                            ...
                            $result = $s3->putObject([
                                'Bucket' => settings()->offload->storage_name,
                                'Key' => UPLOADS_URL_PATH . 'backgrounds/' . $background_new_name,
                                'ContentType' => mime_content_type($file_temp),
                                'SourceFile' => $file_temp,
                                'ACL' => 'public-read'
                            ]);
            ...
            if(isset($_POST['biolink_background_image_remove'])) {
                ...
                if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                    ...
                    $s3->deleteObject([
                        'Bucket' => settings()->offload->storage_name,
                        'Key' => UPLOADS_URL_PATH . 'backgrounds/' . $biolink_theme->settings->biolink->background,
                    ]);
                }
